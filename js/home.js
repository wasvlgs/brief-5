        ...
        .then((products,index) => {
            ...
            for(i = 0; i<4; i++){
                ...
                <div class="w-full bg-white rounded-2xl p-4   shadow-md ">
                                <a href="pages/detail.html"><img onclick="toDetailProduct(${products[i].id})" src="${products[i].image}" alt="${products[i].name}" class="w-full sm:h-40 h-20  object-cover rounded-md cursor-pointer"></a>
                                <div class="flex flex-col justify-end items-end mt-2 cursor-pointer">
                                    <a href="pages/detail.html"><img  onclick="toDetailProduct(${products[i].id})" src="./img/home/Add.png" alt="add" class="w-10 sm:w-10 md:w-12 h-6 sm:h-10 md:h-8"></a>
                                </div>
            ...
            for (let i = 0; i < 8; i++) {
                productGrid.innerHTML += `
                    <div class=" bg-white cursor-pointer rounded-l flex-shrink-0 h-[400px] flex flex-col items-center justify-around  max-w-[230px] px-4 py-2">
                        ...
                        <a href="pages/detail.html"><button onclick="toDetailProduct(${products[i].id})" class=" bg-[#0F67B1] text-white font-semibold py-2 px-6 mx-10  rounded-full hover:bg-blue-600 transition duration-200">Discover</button></a>    
                    </div>
                `;
            }
        })
            ...
            products.slice(0, 10).forEach(product => {
                productGrid.innerHTML += `
                    <div class="carousel-item cursor-pointer rounded-l flex-shrink-0 h-[400px] flex flex-col items-center justify-between bg-white px-4 py-2">
                        ...
                        <a href="pages/detail.html"><button onclick="toDetailProduct(${product.id})" class="sm:mt-4  bg-[#0F67B1] text-white font-semibold px-6 py-2 mb-2 mx-10 rounded-full hover:bg-blue-600 transition duration-200">Discover</button></a>
                    </div>
                `;
            });
        })
